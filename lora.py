...
class Lora(nn.Module):
    def __init__(
        ...
        self.bias = nn.Parameter(torch.empty(out_features) if bias is None else bias)
...
def apply_lora_to_model(
    ...
    """
    will be applied to to layers if layers list is provied if not will be applied to all layers
    """
    ...
    if isinstance(model, (nn.Module, nn.ModuleDict)):
        for key, value in model._modules.items():
            if isinstance(value, nn.Linear):
                if layers is None or key in layers:
                    model._modules[key] = get_lora(value, rank, lora_alpha, device)
            else:
                apply_lora_to_model(value, rank, lora_alpha, layers, device)
    ...
    if isinstance(model, (nn.ModuleList, nn.Sequential)):
        for sub_model in model:
            if isinstance(sub_model, nn.Linear):
                ...
                apply_lora_to_model(sub_model, rank, lora_alpha, layers, device)
